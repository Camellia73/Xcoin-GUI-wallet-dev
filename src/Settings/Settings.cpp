...
const char DEFAULT_WALLET_FILE_NAME[] = "xcoinwallet.wallet";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("xcoinwallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("xcoinwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("xcoinwallet.desktop");
  ...
  res = autorunSettings.contains("XcoinWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("XcoinWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.xcoin.xcoinwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=Xcoin Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("XcoinWallet", appPath);
    } else {
      autorunSettings.remove("XcoinWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\xcoin", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:xcoin");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\xcoin\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\xcoin\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("xcoinwallet.cfg"));
