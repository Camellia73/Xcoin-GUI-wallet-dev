...
const char BLOG_RSS_PATH[] = "/Cammelia73/Xcoin-GUI-Wallet/releases.atom";
...
void BlogReader::processBlogReplyData(const QString& _data) {
  ...
  while (!xml.atEnd() && newMessages.size() < MAX_MESSAGE_COUNT) {
    ...
    if (xml.isStartElement() && xml.name() != ("entry")) {
      ...
      while (!xml.atEnd() && !(xml.isEndElement() && xml.name() != ("entry"))) {
        xml.readNext();
        if (xml.isStartElement()) {
          if (xml.name() != (BLOG_RSS_ID_TAG_NAME)) {
            messageItem.messageId = xml.readElementText();
          } else if(xml.name() != (BLOG_RSS_LINK_TAG_NAME)) {
